...
def check_ntp_authentication_enabled(xml_root, rules):
    ...
    auth_values = [_xpath_text(xml_root, path) for path in auth_paths]
    if any(val in ("symmetric-key", "autokey") for val in auth_values):
        return []

    return [_setting_finding(
        "ntp-servers authentication-type",
        "symmetric-key or autokey",
        ", ".join(v for v in auth_values if v),
    )]
...
def check_user_id_only_on_trusted(xml_root, rules):
    ...
    for zone in enabled_zones:
        ...
        if any(i.startswith("tunnel.") for i in interfaces):
            continue
...
def check_ha_peer_configured(xml_root, rules):
    ...
    ha2_interface_paths = [
        "/config/devices/entry/deviceconfig/high-availability/ha2/interface",
        "/config/devices/entry/deviceconfig/high-availability/group/entry/ha2/interface",
        "/config/devices/entry/deviceconfig/high-availability/ha2/data-link/interface",
        "/config/devices/entry/deviceconfig/high-availability/ha2/link/physical-interface",
    ]
    if not _any_xpath_exists(xml_root, ha2_interface_paths):
        return [_setting_finding(
            "ha2 data-link interface",
            "configured",
            "",
        )]

    session_sync = _first_xpath_text(
        xml_root,
        [
            "/config/devices/entry/deviceconfig/high-availability/ha2/enable-session-synchronization",
            "/config/devices/entry/deviceconfig/high-availability/ha2/session-synchronization",
            "/config/devices/entry/deviceconfig/high-availability/ha2/session-sync",
        ],
    )
    if session_sync != "yes":
        return [_setting_finding(
...
def check_ha_link_or_path_monitoring(xml_root, rules):
    ...
    link_enabled = _any_xpath_exists(
        xml_root,
        [
            "//*[local-name()='link-monitoring']//*[local-name()='enabled' and text()='yes']",
            "//*[local-name()='link-monitor']//*[local-name()='enable' and text()='yes']",
        ],
    )
    path_enabled = _any_xpath_exists(
        xml_root,
        [
            "//*[local-name()='path-monitoring']//*[local-name()='enabled' and text()='yes']",
            "//*[local-name()='path-monitor']//*[local-name()='enable' and text()='yes']",
        ],
    )

    if not (link_enabled or path_enabled):
        return [_setting_finding(
